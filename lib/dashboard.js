...
export async function getRecentEmailStats(userId) {
  try {
    ...
    trackingData?.forEach(record => {
      const campaign = record.campaign || 'Unknown Campaign'
      ...
      if (record.action === 'open') {
        campaignStats[campaign].opens++
        campaignStats[campaign].uniqueOpens.add(record.email_address)
      } else if (record.action === 'click') {
        campaignStats[campaign].clicks++
        campaignStats[campaign].uniqueClicks.add(record.email_address)
      }
      
      // Update last activity
      if (new Date(record.created_at) > new Date(campaignStats[campaign].lastActivity)) {
        campaignStats[campaign].lastActivity = record.created_at
      }
    })

    // Convert to array and calculate rates
    const recentStats = Object.values(campaignStats)
      .map(stat => {
        // Since we don't have actual sent count, we'll use opens as the base
        // Open rate would be 100% since we're only tracking opens
        // Click rate is calculated as clicks / opens
        const openRate = 100 // Since we only track opens, rate is 100%
        const clickRate = stat.opens > 0 ? ((stat.clicks / stat.opens) * 100).toFixed(1) : 0
        
        return {
          id: stat.id,
          name: stat.name,
          sent: stat.opens.toLocaleString(), // Using opens as proxy for sent count
          openRate: `${openRate}%`,
          clickRate: `${clickRate}%`,
          sentDate: new Date(stat.lastActivity).toLocaleDateString()
        }
      })
