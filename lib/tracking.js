...
export function generateSimpleTrackingPixel(action = 'open', emailAddress, campaign, firstName = null, lastName = null) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.honestaffordablehealthcare.com'
...
export function addSimpleTrackingToEmail(html, action = 'open', emailAddress, campaign, firstName = null, lastName = null) {
  let trackedHtml = html

  // Add simple tracking pixel at the end of the email
  const trackingPixel = `<img src="${generateSimpleTrackingPixel(action, emailAddress, campaign, firstName, lastName)}" width="1" height="1" style="display:none;" alt="" />`
  ...
  console.log('Tracking Pixel URL:', generateSimpleTrackingPixel(action, emailAddress, campaign, firstName, lastName))
  console.log('HTML contains tracking pixel:', trackedHtml.includes(generateSimpleTrackingPixel(action, emailAddress, campaign, firstName, lastName)))
  console.log('==================================')
